---
title: API reference
description: Full signatures and validation behavior for every koota-kit export.
---
...
this package's persistence boundary. Restore validates the complete
replacement before mutating the handle.

### Koota exports

`createActions`, `relation`, and `trait` are re-exported so simulation code
can keep its Koota imports at one boundary. `Entity` and `World` are exported
as types.
...
See [architecture notes](/koota-kit/architecture/) for module boundaries and
invariants.
